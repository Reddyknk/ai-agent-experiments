...
async def run_free_mcp_server():
    ...
    print("*** 1. Connecting to the Git MCP Server...")
    ...
    async with stdio_client(server_params) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            # List the available tools provided by this free server
            tools_response = await session.list_tools()
            print("\n*** 2. Available Tools on this Server ---")
            for tool in tools_response.tools:
                print(f"- {tool.name}: {tool.description}")
            
            # Programmatically execute one of the free tools
            # Let's call the 'git_log' tool provided by this server (requires 'repo_path')
            from pathlib import Path
            repo_path = str(Path(__file__).resolve().parents[2])
            print(f"\n*** 3. Executing 'git_log' tool call on repo: {repo_path}...")
            ...
            print("Server Execution Response ---")
