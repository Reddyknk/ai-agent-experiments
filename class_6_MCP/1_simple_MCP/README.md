...
2. **Client Implementations**:
   ...
   - **Client 2: Low-Level Raw JSON-RPC Client (`2_mcp_client_raw.py`)**: Directly interacts with the MCP server over raw stdio subprocess pipes using raw JSON-RPC 2.0 messages, performing the mandatory protocol handshake (`initialize` -> `notifications/initialized` -> `tools/call`).
   ...
   - **Client 4: Remote GitHub MCP Client (`4_mcp_client_github.py`)**: Connects to the official `@modelcontextprotocol/server-github` via `npx`, reading `GITHUB_PERSONAL_ACCESS_TOKEN` from `.env`, and fetches repository files via `get_file_contents`.
   - **Client 5: LLM Model Client (`5_mcp_client_model.py`)**: Bridges MCP tools directly into Google Gemini using `FastMCP` and `google-genai`. Gemini autonomously decides when and how to call tools, returning a natural language answer.

Clients 1 & 5 run an interactive loop prompting `"How many dice rolls should be made: "` and continue until the user enters a non-number (e.g., `q` or `exit`).
...
- [2_mcp_client_raw.py](file:///home/pi-net/Documents/agent_eng_labs/ai-agent-experiments/class_6_MCP/1_simple_MCP/2_mcp_client_raw.py): Low-level raw JSON-RPC 2.0 client communicating over raw subprocess stdin/stdout pipes.
...
- [4_mcp_client_github.py](file:///home/pi-net/Documents/agent_eng_labs/ai-agent-experiments/class_6_MCP/1_simple_MCP/4_mcp_client_github.py): GitHub MCP client running `@modelcontextprotocol/server-github` via `npx` with authentication via `.env`.
- [5_mcp_client_model.py](file:///home/pi-net/Documents/agent_eng_labs/ai-agent-experiments/class_6_MCP/1_simple_MCP/5_mcp_client_model.py): LLM agent client managing the MCP session and delegating tool calling to Gemini via `google-genai`.
...
- A **Google Gemini API Key** (required for `5_mcp_client_model.py`, get one from [Google AI Studio](https://aistudio.google.com/))
- A **GitHub Personal Access Token** (required for `4_mcp_client_github.py`, get one from GitHub Settings -> Developer Settings)
...
- **Node.js & `npx`** (required for `4_mcp_client_github.py` to run `@modelcontextprotocol/server-github`)
...
### Client 4: Remote GitHub MCP Client (`4_mcp_client_github.py`)
This client connects to GitHub's MCP server via `npx` using your personal access token:

```bash
python 4_mcp_client_github.py
```

**What it does:**
1. Loads `GITHUB_PERSONAL_ACCESS_TOKEN` securely from `.env`.
2. Spawns `@modelcontextprotocol/server-github` via `npx -y`.
3. Calls the `get_file_contents` tool to retrieve source files directly from a target GitHub repository, extracting and printing the formatted source code from `"content"`.

---

### Client 5: LLM Model Client (`5_mcp_client_model.py`)
This client uses Google Gemini with the MCP server session registered as live tools.

```bash
python 5_mcp_client_model.py
```
...
In [5_mcp_client_model.py](file:///home/pi-net/Documents/agent_eng_labs/ai-agent-experiments/class_6_MCP/1_simple_MCP/5_mcp_client_model.py), compatibility bridges are applied for smooth integration between `FastMCP` and `google.genai`:
...
In [1_mcp_client_tool.py](file:///home/pi-net/Documents/agent_eng_labs/ai-agent-experiments/class_6_MCP/1_simple_MCP/1_mcp_client_tool.py) and [2_mcp_client_raw.py](file:///home/pi-net/Documents/agent_eng_labs/ai-agent-experiments/class_6_MCP/1_simple_MCP/2_mcp_client_raw.py), `sys.executable` is used to launch the server sub-process with the same active Python interpreter environment.
