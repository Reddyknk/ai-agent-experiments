...
- Get the list of active LLM text generation models that be used by the agent from Google AI Studio API. Use the list in the dropdown menu in the Chat page.
...
- Upon start up, the app should scan the skills/ folder and load the skills that are not currently in the skills database.
- The skills should have a separate vector database should be built as follow:
  - The contents from name, description, and Trigger Queries fields should be sent to the Embedder/Vectorizer.
...
- Create logs for all the invocations and responses between the following components. The log should include the actual details of the payloads passed to and from the components. Store the logs in the database/ folder in log.json file. The log should be in JSON format.
    - agent - full log of the message sent to the agent and the response received from the agent.
    - tool - full log of tool message passed to and received from the tool including the actual payload.
    - ollama vector - log the first 50 characters of the text chunk sent for embedding. Log the response from the vectorizer. Do not include the vectors.
    - external API call - log the full payload passed to the API and the full response received from the API.
    - LLM - prompts sent to and response received from the model include the FULL payload.
